...
class VGG19(nn.Module):
    def __init__(self):
        ...
        # TODO: 调用vgg19网络
        a = vgg19(True)  # TODO完成记录: 加载预训练VGG19
        a = a.features
        # TODO: 定义self.layer1为第2层卷积后对应的特征
        self.layer1 = a[:4]  # TODO完成记录: 第2层卷积后的特征
        # TODO: 定义self.layer2为第4层卷积后对应的特征
        self.layer2 = a[4:9]  # TODO完成记录: 第4层卷积后的特征
        # TODO: 定义self.layer3为第8层卷积后对应的特征
        self.layer3 = a[9:18]  # TODO完成记录: 第8层卷积后的特征
        # TODO: 定义self.layer4为第12层卷积后对应的特征
        self.layer4 = a[18:27]  # TODO完成记录: 第12层卷积后的特征
...
if __name__ == "__main__":
    ...
    optimizer = torch.optim.Adam(
        g_net.parameters(), lr=0.001
    )  # TODO完成记录: 使用Adam优化器优化图像转换网络参数
    ...
    batch_size = 1
    data_loader = DataLoader(data_set, batch_size, True, drop_last=True, num_workers=4)
    # TODO：输入的风格图像经过特征提取网络生成风格特征s1-s4
    s1, s2, s3, s4 = net(image_style)  # TODO完成记录: 提取风格图像在4个层次的特征
    ...
    while j <= epochs:
        for i, image in enumerate(data_loader):
            ...
            image_g = g_net(image_c)  # TODO完成记录: 图像转换网络生成风格化图像
            # TODO: 利用特征提取网络提取生成图像的特征out1、out2、out3、out4
            out1, out2, out3, out4 = net(image_g)  # TODO完成记录: 提取生成图像的4层特征
            """计算风格损失"""
            ...
            c1, c2, c3, c4 = net(image_c)  # TODO完成记录: 提取内容图像特征
            ...
            if i % 20 == 0:
                ...
                save_image(
                    torch.cat([image_c, image_g], dim=3), "./out/train/%d.jpg" % i
                )  # TODO完成记录: 按左右拼接形式保存内容图与生成图
        j += 1
        if j % 2:
            # TODO: 将图像转换网络的参数fst_train.pth存储在models文件夹下
            torch.save(
                g_net.state_dict(), "./models/fst_train.pth"
            )  # TODO完成记录: 保存训练得到的模型参数
