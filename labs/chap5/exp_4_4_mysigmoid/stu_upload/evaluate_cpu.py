...
# TODO：导入自定义动态链接库
import op_mysigmoid.my_sigmoid as my_sigmoid
...
class COCODataSet(Dataset):
    def __init__(self):
        ...
        data_dir = Path("data")
        zip_file = data_dir / "train2014_small.zip"
        if not data_dir.exists() or not zip_file.exists():
            data_dir = Path("/data")
            zip_file = data_dir / "train2014_small.zip"
        ...
        self.zip_files = ZipFile(zip_file)
        self.data_set = []
...
class TransNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # out = torch.sigmoid(x)
        out = my_sigmoid.mysigmoid_cpu(x)
...
if __name__ == "__main__":
    ...
    g_net.load_state_dict(
        torch.load(model_dir / "fst.pth", map_location="cpu", weights_only=False)
    )
    ...
    os.makedirs("./out/cpu", exist_ok=True)

    g_net = g_net.eval().float()
