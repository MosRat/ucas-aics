...
class AttModel(nn.Module):
    def __init__(self, hp_, enc_voc, dec_voc):
        ...
        for i in range(self.hp.num_blocks):
            ...
            self.__setattr__('enc_self_attention_%d' % i, multihead_attention(self.hp, self.hp.hidden_units, self.hp.num_heads, self.hp.dropout_rate, causality=False))
        ...
        for i in range(self.hp.num_blocks):
            ...
            self.__setattr__('dec_self_attention_%d' % i, multihead_attention(self.hp, self.hp.hidden_units, self.hp.num_heads, self.hp.dropout_rate, causality=True))
            #TODO：调用多头注意力机制模块，隐藏维度为hidden_units，包含num_heads个注意力头，使用指定的dropout率，不使用掩码
            self.__setattr__('dec_vanilla_attention_%d' % i, multihead_attention(self.hp, self.hp.hidden_units, self.hp.num_heads, self.hp.dropout_rate, causality=False))
            #TODO：调用前馈神经网络模块进行构建，包含一个隐藏层，中间层维度通常为输入hidden_units的4倍，输出层维度恢复到输入
    ...
    def _decoder_inputs(self, x, y):
        ...
        return torch.cat([Variable(input_tensor * 2).long(), y[:, :-1]], dim=-1)  # 2:<S>
    ...
    def encode(self, x):
        ...
        #TODO： 使用正弦位置编码
        if self.hp.sinusoid:
            enc += self.enc_positional_encoding(x, enc)
        else:
            ...
            enc += self.enc_positional_encoding(Variable(enc_positional))
        ...
        for i in range(self.hp.num_blocks):
            ...
            enc = self.__getattr__('enc_self_attention_%d' % i)(enc, enc, enc)
            #实现编码器的Feed Forward机制
            enc = self.__getattr__('enc_feed_forward_%d' % i)(enc)
        return enc

    def decode(self, x, y, enc):
        decoder_inputs = self._decoder_inputs(x, y)
        # Decoder
        #TODO：解码器端的词嵌入
        ...
        #TODO： 使用正弦位置编码
        if self.hp.sinusoid:
            dec += self.dec_positional_encoding(decoder_inputs, dec)
        else:
            ...
            dec_positional = torch.unsqueeze(torch.arange(0, decoder_inputs.size()[1]), 0).repeat(decoder_inputs.size(0), 1).long()
            if (x.is_cuda):
                dec_positional = dec_positional.cuda()
            if (x.device.type == 'mlu'):
                dec_positional = dec_positional.to('mlu')
            #TODO：加上位置编码
            dec += self.dec_positional_encoding(Variable(dec_positional))
    ...
    def predict(self, x, y, enc=None):
        if enc is None:
            enc = self.encode(x)
        dec = self.decode(x, y, enc)
        logits = self.logits_layer(dec)
        probs = F.softmax(logits.view(-1, self.dec_voc), dim=-1)
        _, preds = torch.max(probs, dim=-1)
        return preds.view(y.size())

    def forward(self, x, y):
        # define decoder inputs
        # self.decoder_inputs = torch.cat([Variable(torch.ones(y[:, :1].size()).cuda() * 2).long(), y[:, :-1]], dim=-1)  # 2:<S>
        self.decoder_inputs = self._decoder_inputs(x, y)
        self.enc = self.encode(x)
        self.dec = self.decode(x, y, self.enc)
        ...
        self.preds = self.preds.view(y.size())
