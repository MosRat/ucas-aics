...
def eval(args):
    ...
    with codecs.open(args.log_path, "a", "utf-8") as fout:
        ...
        for i, (x, sources, targets) in enumerate(test_loader):
            ...
            with torch.inference_mode():
                enc_cache = model.encode(x_)
                for j in range(hp.maxlen):
                    _preds = model.predict(x_, preds, enc_cache)
        ...
        print(
            "qps:",
            (min(len(test_dataset), (i + 1) * args.batch_size) / temp_time)
            if temp_time > 0
            else 0,
        )
    ...
    avg_log = os.getenv("AVG_LOG")
    if avg_log:
        with open(avg_log, "a") as train_avg:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batch-size", default=32, type=int, help="evaluation batch size."
    )
    parser.add_argument("--workers", default=4, type=int, help="number of workers.")
    parser.add_argument(
        "--log-path", default="output.txt", type=str, help="evaluation file path."
    )
    parser.add_argument(
        "--dataset-path",
        default="corpora/",
        type=str,
        help="The path of imagenet dataset.",
    )
    parser.add_argument(
        "--iterations", default=-1, type=int, help="Number of training iterations."
    )
    parser.add_argument(
        "--bitwidth",
        default=8,
    ...
    parser.add_argument(
        "--opt_level", type=str, default="O0", help="choose level of mixing precision"
    )
    # TODO:解析命令行输入的参数
