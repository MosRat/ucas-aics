...
int main(int argc, char**argv) {
    ...
    // 加载配置和数据
    if (!load_config("/opt/code_chap8/data/config", rowsA, colsA, colsB) ||
        !load_16bit_matrix("/opt/code_chap8/data/neuron", activations) ||
        !load_16bit_matrix("/opt/code_chap8/data/weight", weights) ||
        !load_32bit_results("/opt/code_chap8/data/result", expected_results)) {
        return 1;
    }
