...
if __name__ == "__main__":
    ...
    g_net.load_state_dict(
        torch.load("./models/fst.pth", map_location="cpu", weights_only=False)
    )  # TODO-FILLED
    print("g_net build PASS!\n")
    # TODO：将g_net模型转化为eval,并转化为浮点类型，输出得到net
    net = g_net.eval().float()  # TODO-FILLED
    ...
    # TODO: 将net转换为静态计算图
    # net_traced: torch.nn.Module = torch.compile(net)  # TODO-FILLED #type:ignore
    net_traced = torch.jit.trace(net, example_forward_input)  # TODO-FILLED
    # TODO-FILLED: 将模型提前拷贝到 MLU，避免循环内重复搬运
    net_mlu = net_traced.to("mlu")
    # net_traced(example_forward_input.to("mlu"))

    for i, image in enumerate(data_group):
        print(f"The {i} image will be predicted.")
        image_c = image.cpu()
        # TODO：将image_c图片拷贝到MLU设备，得到input_image_c
        input_image_c = image_c.to("mlu")  # TODO-FILLED
        # TODO：将net_trace模型拷贝到MLU设备，得到net_mlu
        net_mlu = net_mlu  # TODO-FILLED
        start = time.perf_counter()
        # TODO: 对input_image_c计算 net_mlu,得到image_g_mlu
        with torch.no_grad():  # TODO-FILLED
            image_g_mlu = net_mlu(input_image_c)  # TODO-FILLED
        ...
        save_image(
            torch.cat([image_c, image_g_mlu], dim=3), f"./out/mlu_cnnl_mfus/{i}.jpg"
        )  # TODO-FILLED
